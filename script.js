  ...
  if (contactForm) {
    ...
    function getFieldError(field) {
      ...
      if (isRequired && !validators.required(value)) {
        return customMessage || field.getAttribute('data-error-required') || 'This field is required.';
      }
      ...
      if (type === 'tel' && !validators.tel(value)) {
        return field.getAttribute('data-error-tel') || 'Please enter a valid phone number.';
      }
    ...
    function showFormMessage(form, type) {
      let msgEl = form.querySelector('.form-submission-message');
      if (!msgEl) {
        msgEl = document.createElement('div');
        msgEl.className = 'form-submission-message';
        msgEl.setAttribute('role', 'alert');
        msgEl.setAttribute('aria-live', 'assertive');
        form.appendChild(msgEl);
      }

      msgEl.className = 'form-submission-message form-submission-message--' + type;

      if (type === 'success') {
        msgEl.textContent = form.getAttribute('data-success-message') || 'Your message has been sent successfully!';
      } else {
        msgEl.textContent = form.getAttribute('data-error-message') || 'Something went wrong. Please try again.';
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שעוני יוקרה בחצי מחיר משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'bc221a02-834a-4c71-ae73-d1f669535353',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'bc221a02-834a-4c71-ae73-d1f669535353';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'bc221a02-834a-4c71-ae73-d1f669535353';
